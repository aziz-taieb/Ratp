...
dj*consdji(sommet***tab){
	uint32_t i,j,k,nbsommet=0;
	for(i=0;i<304;i++){
		for(uint32_t j=0;j<tab[i][0]->nbligne;j++){
			nbsommet++;
		}
	}
	nbsommet++;
	...
	while (fscanf(file, "%d,%d,%s", &num1, &num2, num3) == 3) {
		...
		d->cout[trouversommet(tab,num1,nbsommet)+j][trouversommet(tab,num2,nbsommet)+k]=DIRECTCOST;
	}
	for(i=0;i<nbsommet;i++){
		for(j=0;j<nbsommet;j++){
			if(i==j){
				...
				d->ad[i][j]=1;
				d->ad[j][i]=1;
				d->cout[i][j]=CHANGECOST;
				d->cout[j][i]=CHANGECOST;
			}
		}
	}
	fclose(file);
...
int32_t* dijkstra(uint32_t**G,uint32_t**cost,uint32_t n,uint32_t startnode,uint32_t u,uint32_t nbligne)
{
	...
	/*printf("Distance du noeud faux=%d\n",distance[u+z+2]);*/
	int32_t*tab=malloc(sizeof(int32_t)*(distance[u+z]+3));
	tab[0]=u+z;
	uint32_t ind,k=1;
	uint32_t j=u+z;
	while(j!=startnode){
		min=9999;
		for(i=0;i<388;i++){
			if((cost[i][j]==DIRECTCOST||cost[i][j]==0)&&distance[i]<min){
				min=distance[i];
				ind=i;
			}else if(cost[i][j]==CHANGECOST&&distance[i]+CHANGECOST<min){
				min=distance[i]+CHANGECOST;
				ind=i;
			}
		}
		j=ind;
...
void detruiredj(dj*d,uint32_t nbsommet){
	for(uint32_t i=0;i<nbsommet+1;i++){
		free(d->ad[i]);
		free(d->cout[i]);
	}
	free(d->ad);
